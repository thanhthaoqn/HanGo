# Feature Specification: FE-03 - Role & Permission Management

## 1. Business Context
Data is the goldmine of an EdTech platform. The Analytics Dashboard provides Admins and Course Managers with visual reports (Charts, Statistics) on the system's operational performance: Total new learners, most enrolled courses, exam pass/fail rates. This data empowers the administration board to make decisions on improving content quality.

## 2. Acceptance Criteria

**Frontend (Flutter):**
- [ ] Dedicated Dashboard screen for Admins/Course Managers at `lib/presentation/pages/dashboard/`.
...
- **Backend:** Strict authorization using `@PreAuthorize("hasAnyRole('ADMINISTRATOR', 'TRAINER_LEAD')")`. Learners and Trainers cannot view the system-wide dashboard (Trainers might have a localized dashboard just for their courses).
