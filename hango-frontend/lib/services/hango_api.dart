...
class HangoApi {
  ...
  Future<dynamic> _send(Future<http.Response> request) async {
    ...
    try {
      ...
      throw const ApiFailure(
        'Failed to connect to backend. Please check Spring Boot server and base URL.',
      );
    }
  ...
  Future<void> updateExamInfo(int examId, Map<String, dynamic> data) async {
    await _send(
      http.put(
        _uri('/api/v1/trainer/exams/$examId/info'),
        headers: _headers,
  ...
  Future<void> toggleQuestionStatus(
    int questionId,
    String newStatus, {
    bool isGroup = false,
  }) async {
  ...
  Future<Map<String, dynamic>> getCourseManagerQuestionDetail(
    int id, {
    bool isGroup = false,
  }) async {
  ...
  Future<void> updateCourseManagerQuestionGroup(
    int id,
    ...
    await _send(http.put(uri, headers: _headers, body: jsonEncode(payload)));
  }

  Future<List<Map<String, dynamic>>> getSystemParameters(String type) async {
    final body = await _send(
      http.get(
        _uri('/api/v1/metadata/parameters?type=$type'),
        headers: _headers,
      ),
    );
    return (body as List? ?? const [])
  ...
  Future<void> createCourseManagerQuestionGroup(
    Map<String, dynamic> payload,
  ) async {
    await _send(
      http.post(
  ...
  Future<Map<String, dynamic>> importExamExcel(
    Uint8List fileBytes,
    String fileName,
  ) async {
  ...
  Future<void> deleteExam(int id) async {
    await _send(
      http.delete(_uri('/api/v1/trainer/exams/$id'), headers: _headers),
    );
  }
  ...
  Future<int> countAvailableQuestions(
    ...
    final response = await _send(
      http.get(
        _uri(
          '/trainer/matrices/count-available?skillId=$skillId&diffId=$diffId&catId=$catId',
        ),
        headers: _headers,
      ),
    );
