...
class TrainerTaskPage extends StatefulWidget {
  const TrainerTaskPage({super.key});

  @override
  State<TrainerTaskPage> createState() => _TrainerTaskPageState();
}
...
class _TrainerTaskPageState extends State<TrainerTaskPage> {
  ...
  Future<void> _fetchTasks() async {
    ...
    String? fromDateStr = _fromDateController.text.isNotEmpty
        ? '${_fromDateController.text}T00:00:00Z'
        : null;
    String? toDateStr = _toDateController.text.isNotEmpty
        ? '${_toDateController.text}T23:59:59Z'
        : null;
                        ...
                        Container(
                          ...
                          child: DropdownButtonHideUnderline(
                            child: DropdownButton<String>(
                              ...
                              items: _taskTypes.map<DropdownMenuItem<String>>((
                                String value,
                              ) {
                        ...
                        InkWell(
                          onTap: () async {
                            final picked = await showDatePicker(
                              context: context,
                              initialDate: _assignDeadline,
                              firstDate: DateTime.now(),
                              lastDate: DateTime(2030),
                            );
                            if (picked != null)
                              setState(() => _assignDeadline = picked);
                          },
                          child: Container(
                            height: 48,
                            padding: const EdgeInsets.symmetric(horizontal: 16),
                        ...
                        Container(
                          ...
                          child: DropdownButtonHideUnderline(
                            child: DropdownButton<String>(
                              ...
                              items: _trainers
                                  .where(
                                    (t) => (t['roles'] as List).contains(
                                      'TRAINER',
                                    ),
                                  )
                                  .map<DropdownMenuItem<String>>((trainer) {
                                    return DropdownMenuItem<String>(
                        ...
                        Container(
                          ...
                          child: DropdownButtonHideUnderline(
                            child: DropdownButton<String>(
                              ...
                              items: _trainers.map<DropdownMenuItem<String>>((
                                trainer,
                              ) {
                                return DropdownMenuItem<String>(
                                  value: trainer['id'].toString(),
                    ...
                    onPressed: () async {
                      if (_titleController.text.isEmpty ||
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: Text(
                              'Please fill all required fields and select an assignee and reviewer.',
                            ),
                          ),
                        );
                        return;
    ...
    if (res['success']) {
      ...
      setState(() {
        _activities = activitiesJson
            .map((json) => TaskActivityModel.fromJson(json))
            .toList();
      });
    } else {
      setState(() {
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          Expanded(flex: 2, child: _buildTimelineHistory()),
        ],
      ),
    );
  }
            ...
            Row(
              children: [
                CircleAvatar(
                  ...
                  child: Text(
                    assignee.creatorName[0].toUpperCase(),
            ...
            Row(
              children: [
                CircleAvatar(
                  ...
                  child: Text(
                    assignee.reviewerName![0].toUpperCase(),
                    style: const TextStyle(color: Colors.white, fontSize: 14),
                  ),
                ),
                const SizedBox(width: 12),
    ...
    return Column(
      ...
      children: [
        ...
        ..._activities.map((activity) {
          bool isCritical =
              activity.action.toLowerCase().contains('reject') ||
              activity.action.toLowerCase().contains('error');
          return Padding(
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                Builder(
                                  builder: (context) {
                                    bool canEditReviewer =
                                        DateTime.now().isBefore(task.dueDate) &&
                              ...
                              ElevatedButton(
                                onPressed: () async {
                                  final assignee = task.assignees.isNotEmpty
                                      ...
                                      : null;
                                  final newReviewerId = _editReviewerId != null
                                      ? int.tryParse(_editReviewerId!)
                                      : assignee?.reviewerId;
                                  if (newReviewerId != null &&
                                      newReviewerId == assignee?.creatorId) {
                                    ScaffoldMessenger.of(context).showSnackBar(
                                      const SnackBar(
                                        content: Text(
                                          'Assignee and Reviewer cannot be the same.',
                                  ...
                                  final data = {
                                    ...
                                    'type': task.type,
                                    'dueDate':
                                        _editDeadline?.toIso8601String() ??
                                        task.dueDate.toIso8601String(),
                                    'assigneeId': assignee?.creatorId,
                                    'reviewerId': newReviewerId,
                                  };

                                  final updateRes = await _taskService
                                      .updateTask(task.id, data);
                                  if (updateRes['success'] == true) {
                                    if (_editStatus != null &&
                                        _editStatus != assignee?.status) {
                                    ...
                                    if (mounted) {
                                      ScaffoldMessenger.of(
                                        ...
                                        SnackBar(
                                          content: Text(
                                            'Failed to update: ${updateRes['message']}',
                                          ),
                                        ),
                                      );
                                    }
              ...
              Container(
                ...
                child: const Row(
                  children: [
                    ...
                    Expanded(
                      flex: 2,
                      child: Text('ASSIGNEE', style: _headerStyle),
                    ),
                    Expanded(
                      flex: 2,
                      child: Text('REVIEWER', style: _headerStyle),
                    ),
                      ...
                      child: Row(
                        children: [
                            ...
                            child: Row(
                              children: [
                                ...
                                Expanded(
                                  child: Text(
                                    task.assignees.isNotEmpty
                                        ? task.assignees.first.creatorName
                                        : 'N/A',
                          ...
                          Expanded(
                            ...
                            child: Text(
                              task.assignees.isNotEmpty &&
                                      task.assignees.first.reviewerName != null
                                  ? task.assignees.first.reviewerName!
                                  : 'N/A',
                              style: const TextStyle(fontSize: 12),
                              overflow: TextOverflow.ellipsis,
                            ),
                          ...
                          Expanded(
                            ...
                            child: Container(
                              ...
                              child: Text(
                                task.assignees.isNotEmpty
                                    ? task.assignees.first.status.replaceAll(
                                        '_',
                                        ' ',
                                      )
                                    : 'ASSIGNED',
                                ...
                                IconButton(
                                  ...
                                  onPressed: () {
                                    setState(() {
                                      ...
                                      _editStatus = task.assignees.isNotEmpty
                                          ? task.assignees.first.status
                                          : 'ASSIGNED';
                                      _editReviewerId =
                                          task.assignees.isNotEmpty
                                          ? task.assignees.first.reviewerId
                                                ?.toString()
                                          : null;
                                    });
                                    _fetchTaskActivities(task.id);
