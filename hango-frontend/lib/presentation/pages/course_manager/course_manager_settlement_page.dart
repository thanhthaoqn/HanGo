...
class _CourseManagerSettlementPageState
  ...
  void _showSettleDialog(
    ...
    final receiptUrlController = TextEditingController(
      text: isReadOnly ? (statement['payoutReceiptUrl']?.toString() ?? '') : '',
    );
              ...
              try {
                ...
                if (response.statusCode == 200 || response.statusCode == 201) {
                  ...
                  if (secureUrl != null) {
                    ...
                    if (context.mounted) {
                      ToastHelper.showSuccess(
                        context,
                        isVi
                            ? 'Đã tải ảnh Bill chuyển tiền thành công!'
                            : 'Payout receipt proof uploaded successfully!',
                      );
                    }
                  }
                } else {
                  if (context.mounted) {
                    ToastHelper.showError(
                      context,
                      'Cloudinary upload error: ${response.statusCode}',
                    );
                  }
                }
              } catch (e) {
              ...
              title: Row(
                ...
                children: [
                  Text(
                    isReadOnly
                        ? (isVi
                              ? 'Chứng từ Chuyển khoản (Đã thanh toán)'
                              : 'Payout Transfer Record (PAID)')
                        : (isVi
                              ? 'Xác nhận Đã chuyển khoản'
                              : 'Mark as Paid / Record Transfer'),
                ...
                child: SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                      Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            const SizedBox(height: 8),
                            _buildDialogRow(
                              isVi ? 'Tên chủ tài khoản:' : 'Account Owner:',
                              bankAccountName,
                            ),
                      ...
                      Text(
                        isVi
                            ? 'Bằng chứng chuyển khoản (Ảnh Bill) *'
                            : 'Payout Receipt Proof Image *',
                        style: const TextStyle(
                          fontWeight: FontWeight.bold,
                          fontSize: 13,
                          color: Color(0xFF334155),
                      ...
                      if (receiptUrlController.text.trim().isEmpty) ...[
                        InkWell(
                          onTap: isReadOnly || isUploadingReceipt
                              ? null
                              : handleUploadReceipt,
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                Text(
                                  isUploadingReceipt
                                      ? (isVi
                                            ? 'Đang tải ảnh Bill lên hệ thống...'
                                            : 'Uploading receipt image...')
                                      : (isVi
                                            ? 'Bấm để Tải ảnh Bill chuyển tiền (PNG, JPG)'
                                            : 'Click to Upload Receipt Bill Image (PNG, JPG)'),
                                  style: const TextStyle(
                                    fontSize: 13,
                                    fontWeight: FontWeight.bold,
                                    color: Color(0xFF334155),
              ...
              actions: [
                if (isReadOnly) ...[
                  ...
                  ElevatedButton(
                        ...
                        if (success) {
                          ToastHelper.showSuccess(
                            ...
                            isVi
                                ...
                                : 'Statement marked as PAID successfully!',
                          );
                          _fetchStatements();
                          _fetchPayments();
                        } else {
                    ...
                    child: Text(
                      isVi ? 'Xác nhận Đã chuyển' : 'Confirm Paid',
                      style: const TextStyle(fontWeight: FontWeight.bold),
                    ),
                  ),
                ],
        ...
        return AlertDialog(
          ...
          title: Row(
            children: [
              ...
              Text(
                isVi
                    ? 'Tính toán & Chốt lại Báo cáo'
                    : 'Recalculate & Resubmit Statement',
                style: const TextStyle(
                  fontWeight: FontWeight.bold,
                  fontSize: 16,
                  fontFamily: 'Outfit',
                  ...
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        Text(
                          isVi
                              ? 'Lý do từ chối trước đó / Ghi chú:'
                              : 'Previous Rejection Reason / Notes:',
                          style: const TextStyle(
                            fontSize: 12,
                            fontWeight: FontWeight.bold,
          ...
          actions: [
            ...
            ElevatedButton.icon(
              onPressed: () async {
                Navigator.pop(context);
                setState(() => _isLoading = true);
                final success = await _revenueService.regenerateStatement(
                  statementId,
                );
                if (mounted) {
                  setState(() => _isLoading = false);
                  if (success) {
                    ToastHelper.showSuccess(
                      context,
                      isVi
                          ? 'Đã tính toán & gửi lại Báo cáo thành công!'
                          : 'Statement recalculated & resubmitted successfully!',
                    );
                    _fetchStatements();
                  } else {
                    ToastHelper.showError(
                      context,
                      isVi
                          ? 'Không thể chốt lại báo cáo. Vui lòng thử lại.'
                          : 'Failed to recalculate statement.',
                    );
                  }
                }
              },
              ...
              label: Text(
                isVi ? 'Xác nhận Chốt lại' : 'Confirm Resubmit',
                style: const TextStyle(fontWeight: FontWeight.bold),
              ),
              style: ElevatedButton.styleFrom(
