...
public class EntryExamDataInitializer implements CommandLineRunner {
  ...
  public void run(String... args) throws Exception {
    ...
    Integer count = jdbcTemplate.queryForObject("SELECT COUNT(*) FROM exams WHERE id = 999", Integer.class);
    boolean examExists = (count != null && count > 0);

    if (examExists) {
      Integer qCount = jdbcTemplate.queryForObject("SELECT COUNT(*) FROM exam_questions WHERE exam_id = 999",
          Integer.class);
      if (qCount != null && qCount >= 40) {
        log.info("Entry Exam (id=999) already exists with full questions. Skipping initialization.");
        return;
      } else {
        log.info("Entry Exam (id=999) is incomplete or needs replacement. Clearing old linkages...");
        jdbcTemplate.update("DELETE FROM exam_questions WHERE exam_id = 999");
      }
    }
    ...
    if (examExists) {
      log.info("Updating existing Entry Exam (id=999)...");
      jdbcTemplate.update(
          "UPDATE exams SET title = 'Global Entry Placement Test', description = 'A comprehensive exam to assess all 25 skill domains and provide a personalized learning pathway.', duration_minutes = 50, status = 'PUBLISHED', expected_question_count = 40, visibility = 'PUBLIC', passing_score = 5.0, thumbnail_url = 'https://res.cloudinary.com/diqekap4o/image/upload/v1714578160/hango-assets/exam-banner.png' WHERE id = 999");
    } else {
      log.info("Inserting Entry Exam (id=999)...");
      jdbcTemplate.update(
          "INSERT INTO exams (id, title, description, duration_minutes, status, expected_question_count, visibility, created_by, created_at, version, passing_score, thumbnail_url) "
              +
              "VALUES (999, 'Global Entry Placement Test', 'A comprehensive exam to assess all 25 skill domains and provide a personalized learning pathway.', 50, 'PUBLISHED', 40, 'PUBLIC', ?, NOW(), 'v1.0', 5.0, 'https://res.cloudinary.com/diqekap4o/image/upload/v1714578160/hango-assets/exam-banner.png')",
          userId);
    }
    ...
    log.info("Entry Exam (id=999) successfully generated with 40 questions.");
  }
