...
public interface PaymentRepository extends JpaRepository<Payment, Long> {
    ...
    @org.springframework.data.jpa.repository.Query("SELECT p FROM Payment p JOIN FETCH p.course c JOIN FETCH c.creator crt LEFT JOIN FETCH c.category LEFT JOIN FETCH c.difficulty WHERE crt.id = :creatorId AND p.status = :status ORDER BY p.createdAt DESC LIMIT 5")
    List<Payment> findTop5ByCourseCreatorIdAndStatusOrderByCreatedAtDesc(@org.springframework.data.repository.query.Param("creatorId") Long creatorId, @org.springframework.data.repository.query.Param("status") String status);
