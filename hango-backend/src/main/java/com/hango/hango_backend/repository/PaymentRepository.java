...
public interface PaymentRepository extends JpaRepository<Payment, Long> {
       ...
       java.math.BigDecimal sumRevenueByTrainerId(
       ...
       List<Payment> findTop20ByCourseCreatorIdAndStatusOrderByCreatedAtDesc(
                     @org.springframework.data.repository.query.Param("creatorId") Long creatorId,
       ...
       @org.springframework.data.jpa.repository.Query(value = "SELECT MONTH(p.created_at) as month, SUM(IFNULL(p.trainer_earnings, 0)) as revenue "
                     ...
                     "GROUP BY MONTH(p.created_at)", nativeQuery = true)
       List<Object[]> getRevenueByMonthForCurrentYear(
                     @org.springframework.data.repository.query.Param("trainerId") Long trainerId);
       ...
       @org.springframework.data.jpa.repository.Query(value = "SELECT MONTH(p.created_at) as month, SUM(IFNULL(p.trainer_earnings, 0)) as revenue "
                     ...
                     "GROUP BY MONTH(p.created_at)", nativeQuery = true)
       List<Object[]> getRevenueByMonthForYear(
                     @org.springframework.data.repository.query.Param("trainerId") Long trainerId,
       ...
       @org.springframework.data.jpa.repository.Query(value = "SELECT DATE(p.created_at) as date, SUM(IFNULL(p.trainer_earnings, 0)) as revenue "
                     ...
                     "GROUP BY DATE(p.created_at)", nativeQuery = true)
       List<Object[]> getRevenueByDay(@org.springframework.data.repository.query.Param("trainerId") Long trainerId,
       ...
       void insertMockPayment(@org.springframework.data.repository.query.Param("amount") java.math.BigDecimal amount,
                     ...
                     @org.springframework.data.repository.query.Param("trainerEarnings") java.math.BigDecimal trainerEarnings,
                     @org.springframework.data.repository.query.Param("platformFee") java.math.BigDecimal platformFee,
                     @org.springframework.data.repository.query.Param("settlementStatus") String settlementStatus,
                     @org.springframework.data.repository.query.Param("txnRef") String txnRef);
       ...
       @Query(value = "SELECT p.trainer_earnings, p.amount, p.settlement_status, p.created_at " +
                     ...
                     "WHERE c.created_by = :trainerId AND p.status = 'SUCCESS'", nativeQuery = true)
       List<Object[]> findRevenueDataByTrainerId(@Param("trainerId") Long trainerId);

       List<Payment> findByCourseCreatorIdAndStatusAndSettlementStatus(Long creatorId, String status,
       ...
       @Query(value = "SELECT p FROM Payment p LEFT JOIN p.user u LEFT JOIN p.course c " +
                     ...
                     "ORDER BY COALESCE(p.paidAt, p.createdAt) DESC, p.id DESC", countQuery = "SELECT COUNT(p) FROM Payment p LEFT JOIN p.user u LEFT JOIN p.course c "
                                   +
                                   "WHERE (:status IS NULL OR :status = '' OR UPPER(p.status) = UPPER(:status)) " +
                                   "AND (:settlementStatus IS NULL OR :settlementStatus = '' " +
                                   "     OR (:settlementStatus = 'PENDING' AND (p.statementId IS NULL OR UPPER(p.settlementStatus) = 'PENDING')) "
